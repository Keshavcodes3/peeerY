...
export default function AppLayout() {
    ...
    // Fetch profile display name so the sidebar shows what user set in Profile Settings
    useEffect(() => {
        if (!user) return;
        api.get<{ success: boolean; profile: { name: string; avatar?: string } }>("/api/v1/profile/me")
            .then(res => {
                if (res.data?.profile) {
                    ...
                    setProfileAvatar(res.data.profile.avatar || null);
                }
            })
    ...
    const displayInitial = profileName ? profileName[0].toUpperCase() : (user?.username?.replace(/[0-9a-f]{8}$/i, '') || "U")[0].toUpperCase();
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="px-3 py-4 border-t border-zinc-100">
                 <Link to="/profile" className="flex items-center gap-3 px-3 py-2 rounded-xl bg-zinc-50 hover:bg-zinc-100 transition-colors mb-2 cursor-pointer">
                    <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-bold uppercase shrink-0 overflow-hidden">
                        {profileAvatar
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-zinc-950 truncate">{displayName}</p>
