...
export default function ProjectWorkspace() {
    ...
    useEffect(() => {
        api.get<{ success: boolean; profile: { name: string } }>("/api/v1/profile/me")
            .then(res => res.data?.profile && setProfileName(res.data.profile.name))
            .catch(() => {});
    }, []);
    ...
    const handleAddTask = (e: React.FormEvent) => {
        ...
        if (newTaskAssignee && !members.some(member => member.user.username === newTaskAssignee)) {
            showToast("No user found with this username in the workspace");
            return;
        }
            ...
            <AnimatePresence>
                {toast && (
                    <motion.div
                        ...
                        className="fixed top-24 left-1/2 z-[60] bg-zinc-950 text-white text-xs font-semibold px-4 py-2.5 rounded-full shadow-xl"
                    >
                        {toast}
                    </motion.div>
                )}
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-bold text-zinc-900 font-display capitalize">
                                                                {m.user?.username === user?.username ? (profileName ?? m.user?.username) : (m.user?.username)}
                                                                {isTargetMe && " (You)"}
                                                            </span>
