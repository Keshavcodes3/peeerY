...
export default function Dashboard() {
    ...
    useEffect(() => {
        if (!user) return;
        api.get<{ success: boolean; profile: { name: string; avatar?: string } }>("/api/v1/profile/me")
            .then(res => {
                if (res.data?.profile) {
                    setProfileName(res.data.profile.name);
                }
            })
            .catch(() => { /* no profile yet — fall back to auth username */ });
    }, [user]);

    // Clean up username for display (remove Clerk ID suffix if present)
    const displayName = profileName || (user?.username?.replace(/[0-9a-f]{8}$/i, '') || "builder");
    ...
    const fetchActivities = async () => {
        try {
            const res = await api.get<{ data: any[] }>("/api/v1/notifications");
            if (res.data?.data) {
                setActivities(res.data.data.map((n: any) => ({
                    id: n._id,
                    title: n.title,
                    detail: n.message,
                    time: "Just now", // In a real app, use createdAt
                    type: n.type
                })));
            }
        } catch (e) {
    ...
    useEffect(() => { loadDashboardData(); fetchActivities(); }, []);
                    ...
                    {[
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-3 text-sm font-bold border-b-2 transition-all shrink-0 cursor-pointer ${activeTab === tab.id
                                    ...
                                    <div className="grid gap-5 md:grid-cols-3">
                                        ...
                                        <div className="md:col-span-2 bg-white border border-zinc-200/80 rounded-2xl p-6 space-y-4">
                                            ...
                                            <div className="divide-y divide-zinc-100">
                                                {activities.length === 0 ? (
                                                    <p className="text-xs text-zinc-400 py-4">No recent activity.</p>
                                                ...
                                                <div key={proj._id} className="bg-white border border-zinc-200/80 rounded-2xl p-5 space-y-4 hover:border-zinc-300 transition-colors flex flex-col justify-between">
                                                    <div className="space-y-2">
                                                        <div className="flex justify-between items-start">
                                                            <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${proj.role === "Owner"
                                                ...
                                                <div key={app._id} className="bg-white border border-zinc-200/80 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-5">
                                                    <div className="space-y-1.5 flex-1">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wider ${app.status === "ACCEPTED" ? "bg-emerald-50 text-emerald-600 border-emerald-100" :
