...
const CONTRIBUTIONS = [
    { repo: "vercel/next.js", title: "Fix hydration mismatch in App Router", stars: "120k", date: "2 days ago", status: "merged" },
    { repo: "shadcn/ui", title: "Add combobox accessibility improvements", stars: "68k", date: "5 days ago", status: "open" },
    { repo: "t3-oss/create-t3-app", title: "Update Drizzle ORM template config", stars: "24k", date: "1 week ago", status: "merged" },
    { repo: "tailwindlabs/tailwindcss", title: "Report: container queries in JIT mode", stars: "81k", date: "2 weeks ago", status: "closed" },
]

const STREAK_DATA = Array.from({ length: 52 }, () => Array.from({ length: 7 }, () => Math.random()))
...
export function ContributionView() {
    return (
        <div className="h-full flex flex-col p-5 sm:p-8 overflow-y-auto">
            <div className="flex items-center justify-between mb-6 sm:mb-8 gap-3">
                <div>
                    <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-zinc-900 flex items-center gap-2">
                        <GitPullRequest className="text-blue-600" size={22} />
                        Contributions
                    </h3>
                    <p className="text-xs sm:text-sm text-zinc-500 mt-1">Your open-source impact, at a glance.</p>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-50 border border-blue-100 text-blue-600 text-xs font-semibold shrink-0">
                    <TrendingUp size={14} />
                    42 this month
                </div>
            </div>

            <div className="mb-6 sm:mb-8 p-4 sm:p-5 bg-zinc-50/50 border border-zinc-100 rounded-2xl overflow-x-auto">
            ...
            <div className="space-y-3">
                {CONTRIBUTIONS.map((contrib, i) => (
                    <motion.div
                        ...
                        whileHover={{ x: 4 }}
                        className="group flex items-start gap-3 sm:gap-4 p-3.5 sm:p-4 bg-white border border-zinc-200 rounded-xl hover:border-blue-200 hover:shadow-md hover:shadow-blue-500/5 transition-all duration-300 cursor-pointer"
                    >
                        <div
                            className={`shrink-0 w-9 h-9 rounded-lg flex items-center justify-center mt-0.5 ${contrib.status === "merged"
