...
export const sanitizeProfileInput = (input: Partial<authProfile>): Partial<authProfile> => {
  ...
  if (input.experience) {
    // Normalize to Title Case to match schema enum ["Beginner", "Intermediate", "God"]
    const exp = input.experience.toLowerCase();
    if (exp === 'god' || exp === 'lead') clean.experience = 'God';
    else if (exp === 'intermediate' || exp === 'expert') clean.experience = 'Intermediate';
