...
import { sendInviteEmail, sendJoinConfirmationEmail } from "../../../Infra/Mail/email.js";
...
const sendInvitation = async (
    ...
    const invitedUser = await authModel.findById(invitedUserId);
    if (invitedUser?.email) {
        await sendInviteEmail(invitedUser.email, project.title, inviterUser?.username || "A builder");
    }
...
const acceptInvitation = async (invitationId: string, userId: string) => {
    ...
    if (invitedUserDoc?.email && projectDoc) {
